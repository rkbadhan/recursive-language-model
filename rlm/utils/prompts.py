...
This module defines the core prompts that guide the RLM's behavior:
...
For specialized domain prompts (log analysis, jstack, etc.), see examples/ directory.
"""
...
def build_system_prompt(custom_prompt: Optional[str] = None) -> List[Dict[str, str]]:
    ...
    Args:
        custom_prompt: Optional custom system prompt to use instead of default.
                      For specialized prompts, see examples/ directory
                      (e.g., jstack_analysis, log_analysis, etc.)

    Returns:
        List containing the system message with REPL instructions
    """
    content = custom_prompt if custom_prompt else REPL_SYSTEM_PROMPT
